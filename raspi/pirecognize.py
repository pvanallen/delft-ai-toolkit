...
def run_inference_on_image(FLAGS,image):
...
def take_picture_recognize(FLAGS, camera):

  # take and write a snapshot to a file
  if take_picture_recognize.picture_being_taken == False:

    # take picture
    take_picture_recognize.picture_being_taken = True
    image = os.path.join(FLAGS.model_dir, 'object-image.jpg')
    print("taking picture...")
    camera.capture(image)
    time.sleep(0.2)
    print("picture ready")
    take_picture_recognize.picture_being_taken = False

    # perform object recognition
    print("analyzing...")
    image = os.path.join(FLAGS.model_dir, 'object-image.jpg')
    match_results = run_inference_on_image(FLAGS,image)
